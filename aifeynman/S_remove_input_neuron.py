...
def remove_input_neuron(net, n_inp, idx_neuron, ct_median, save_filename):
    ...
    # Adjust the biases
    if is_cuda:
        net.linear1.bias = nn.Parameter(
            net.linear1.bias + torch.tensor(ct_median * removed_weights).float().cuda()
        )
    else:
        net.linear1.bias = nn.Parameter(
            net.linear1.bias + torch.tensor(ct_median * removed_weights).float()
        )
