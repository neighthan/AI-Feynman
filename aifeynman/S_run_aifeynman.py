...
def run_AI_all(
    ...
    if succ_grad == 1:
        ...
        for qqqq in range(1):
            brute_force_comp(
                "results/", "gradients_comp_%s.txt" % filename, 600, "14ops.txt"
            )
            ...
            for bf_i in range(len(bf_all_output)):
                ...
                try:
                    ...
                    idx_comp_temp, eqq, new_mu, new_sigma = check_compositionality(
                        pathdir, filename, model_feynman, express, mu, sigma, nu=10
                    )
                    if idx_comp_temp == 1:
                        idx_comp = 1
                        math_eq_comp = eqq
