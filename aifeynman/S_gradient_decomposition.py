...
def evaluate_derivatives_andrew(model, s, pts):
    ...
    pts.requires_grad_(True)
    outs = model(pts)
    grad = torch.autograd.grad(outs, pts, grad_outputs=grad_weights, create_graph=True)[
        0
    ]
...
def visualize_distribution(hypot, bench):
    ...
    kde = KernelDensity(kernel="tophat", bandwidth=0.25).fit(
        np.log10(1 - hypot + eps)[:, np.newaxis]
    )
    kde2 = KernelDensity(kernel="tophat", bandwidth=0.25).fit(
        np.log10(1 - bench + eps)[:, np.newaxis]
    )
    X_plot = np.linspace(-8, 0, 1000)[:, np.newaxis]
    log_dens = kde.score_samples(X_plot)
    log_dens2 = kde2.score_samples(X_plot)
    plt.plot(
        X_plot[:, 0], np.exp(log_dens), color="#FFAA00", label="Hypothesis"
    )  # hypot is orange
    plt.plot(
        X_plot[:, 0], np.exp(log_dens2), color="#00AAFF", label="Benchmark"
    )  # blue is bench
    plt.legend()
...
def get_kde(X_vals, dist, kernel="tophat", bandwidth=0.3):
    ...
    kde = KernelDensity(kernel=kernel, bandwidth=bandwidth).fit(
        np.log10(1 - scores + eps)[:, np.newaxis]
    )
...
def filter_decompositions_relative_scoring(
    X, y, model, max_subset_size=None, visualize=False
):
    """
    X: torch tensor. N * d
    y: torch tensor. N * 1
    ...
    random_indices = np.random.choice(
        X.shape[0], size=min(NUM_TRIALS, X.shape[0]), replace=False
    )
    for s in powerset_atleast_2(range(0, n), max_subset_size=max_subset_size):
        ...
        for i in range(random_indices.shape[0]):
            samples = draw_samples(
                X, y, model, (), NUM_SAMPLES, point=random_indices[i]
            )
            score, _ = score_consistency(evaluate_derivatives_andrew(model, s, samples))
...
def extract_gradients(X, y, model, s, num_points):
    ...
    for i in range(num_points):
        ...
        _, gradients[i, :] = score_consistency(
            evaluate_derivatives_andrew(model, s, samples)
        )
...
def identify_decompositions(
    pathdir, filename, model, max_subset_size=2, visualize=False
):
    ...
    all_scores = filter_decompositions_relative_scoring(
        X, y, model, visualize=visualize
    )
    assert all_scores
