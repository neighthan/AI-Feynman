...
def NN_eval(pathdir, filename):
    try:
        ...
        factors_val = torch.from_numpy(
            variables[int(5 * len(variables) / 6) : int(len(variables))]
        )
        if is_cuda:
        ...
        product_val = torch.from_numpy(
            f_dependent[int(5 * len(variables) / 6) : int(len(variables))]
        )
        if is_cuda:
        ...
        model.load_state_dict(
            torch.load("results/NN_trained_models/models/" + filename + ".h5")
        )
        model.eval()
