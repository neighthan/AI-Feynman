...
def final_gd(data, math_expr, lr=1e-2, N_epochs=5000):
    ...
    def unsnap_recur(expr, param_dict, unsnapped_param_dict):
        ...
        if (
            ...
            or isinstance(expr, sympy.numbers.Rational)
            or isinstance(expr, sympy.numbers.Pi)
        ):
    ...
    for nan_i in range(len(trainable_parameters)):
        if (
            torch.isnan(trainable_parameters[nan_i]) == True
            or abs(trainable_parameters[nan_i]) > 1e7
        ):
    ...
    numbers_expr = [
        subexpression
        for subexpression in preorder_traversal(eq)
